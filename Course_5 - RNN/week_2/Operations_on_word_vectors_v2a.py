...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    b_axes_norm2 = np.dot(bias_axis,bias_axis)
    mu_B = (np.dot(mu,bias_axis) / b_axes_norm2) * bias_axis
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = (np.dot(e_w1,bias_axis) / b_axes_norm2) * bias_axis
    e_w2B = (np.dot(e_w2,bias_axis) / b_axes_norm2) * bias_axis
        
    # Step 5: Adjust the Bias part of e_w1B and e_w2B using the formulas (9) and (10) given above (≈2 lines)
    mu_fac = np.sqrt(np.abs(1-np.dot(mu_orth,mu_orth)))
    corrected_e_w1B = mu_fac * (e_w1B - mu_B)/np.sqrt(np.dot((e_w1-mu_orth - mu_B),(e_w1-mu_orth - mu_B)))
    corrected_e_w2B = mu_fac * (e_w2B - mu_B)/np.sqrt(np.dot((e_w2-mu_orth - mu_B),(e_w2-mu_orth - mu_B)))
